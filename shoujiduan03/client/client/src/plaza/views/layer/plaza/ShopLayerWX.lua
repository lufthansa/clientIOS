...
--
--常见问题页面
local ShopLayerWX = class("ShopLayerWX", cc.Layer)
local ExternalFun = require(appdf.EXTERNAL_SRC .. "ExternalFun")

--返回按钮
local BT_EXIT       = 101
function ShopLayerWX:ctor( scene )
    self._scene = scene

    --加载csb资源
    local rootLayer, csbNode = ExternalFun.loadRootCSB("feedback/WXBuyLayer.csb", self)
...
function ShopLayerWX:onButtonClickedEvent( tag, sender )
    if BT_EXIT == tag then
